#Written By #Shubham
#Ported By Devil (github.com/ITZ-DEVIL-OP ; t.me/DANlSH_OP)
...
from Alita.modules.sql.setbio_sql import set_bio, rm_bio, check_bio_status, is_bio, get_all_bio_id
from Alita.modules.sql.setbio_sql import set_bio, rm_bio
from Alita.modules.sql.setbio_sql import SUDO_USERS as boss
from Alita import tbot, OWNER_ID, CMD_HELP, ubot, StartTime, MONGO_DB_URI, BOT_ID
import datetime, time
from Alita.function import is_admin, bio
...
@register(pattern="^/(ping|ping@Alita_Gbot)")
...
async def gey(event):
   ...
   async with tbot.conversation("@roseloverx") as bot_conv:
      response = bot_conv.wait_event(
                events.NewMessage(incoming=True, from_users="@DANlSH_OP")
            )
      await (await inline_query(ubot, "@HowGayBot", k))[0].click("@Alita_Gbot")
      response = await response
...
async def bgay(event):
   ...
   async with ubot.conversation("@Gayroebot") as bot_conv:
      ...
      s = response.text.replace("◡̈⃝Devil", k)
      p = s.replace("*", "")
      j = p.replace(k, "")
      gey = j.replace("tg://user?id=1883720399", "")
      uf = gey.replace("[]()", "")
 ...
 try:
    ...
    async with ubot.conversation(chat) as conv:
        try:
            ...
            await stt.edit("Error, Report at @AlitaBotSupport")
            return
